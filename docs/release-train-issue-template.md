...
- [ ] Check that GitHub Actions release build has executed successfully (GitHub Actions will start a [CI build](https://github.com/akka/akka-grpc/actions) for the new tag and publish artifacts to the Akka repository)
...
- [ ] Check the release using your token resolver URL from https://account.akka.io/token: `mvn dependency:get -Dartifact=com.lightbend.akka.grpc:akka-grpc-scalapb-protoc-plugin_2.12:$VERSION$ -Dmaven.repo.remote=<token url>`
