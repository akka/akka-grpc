...
abstract class ScalaCodeGenerator extends CodeGenerator {
  ...
  def perServiceContent: Set[(Logger, Service) ⇒ Option[CodeGeneratorResponse.File]] = Set.empty
  ...
  override def run(request: CodeGeneratorRequest, logger: Logger): CodeGeneratorResponse = {
    ...
    for {
      ...
      generator(logger, service).map(b.addFile)
    }
  ...
  private def parseParameters(params: String): ExtendedGeneratorParams = {
    params.split(",").map(_.trim).filter(_.nonEmpty).foldLeft[ExtendedGeneratorParams](ExtendedGeneratorParams()) {
      case (p, "java_conversions") => p.copy(baseParams = p.baseParams.copy(javaConversions = true))
      case (p, "flat_package") => p.copy(baseParams = p.baseParams.copy(flatPackage = true))
      case (p, "grpc") => p.copy(baseParams = p.baseParams.copy(grpc = true))
      case (p, "single_line_to_string") => p.copy(baseParams = p.baseParams.copy(singleLineToProtoString = true))
      case (p, "server_power_apis") => p.copy(serverPowerApis = true)
      case (x, _) => x
    }
  }
}
object ScalaCodeGenerator {
  val generateServiceFile: (Logger, Service) => Option[CodeGeneratorResponse.File] = (logger, service) => {
