  ...
  def createChannel(settings: GrpcClientSettings, log: LoggingAdapter)(
    ...
    new InternalChannel {
      ...
      override def invokeWithMetadata[I, O](
          ...
          options: CallOptions) = {
        val flow =
          createFlow(fqMethodName, headers, descriptor, streamingResponse, callOptionsWithDeadline(options, settings))
      ...
      private def createFlow[I, O](
        ...
        Flow.fromGraph(
          new AkkaNettyGrpcClientGraphStage(descriptor, fqMethodName, channel, options, streamingResponse, headers))
